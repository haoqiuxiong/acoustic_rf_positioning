...
def open_dataset(
    ...
    if dataset_path is None:
        ...
        # Fallback: if an experiment-specific filename is not present (for example when a
        # merged multi-experiment file is used), scan generic CSI files and pick one that
        # actually contains the requested experiment IDs.
        if not candidates and dataset_glob is None and experiment_id is not None:
            all_csi_candidates = find_dataset_paths(
                experiment_id=None,
                dataset_glob=None,
                search_dirs=search_dirs,
            )
            requested_experiment_ids = normalize_experiment_ids(experiment_id)
            for generic_candidate in all_csi_candidates:
                generic_dataset = open_netcdf_dataset(generic_candidate, label="RF dataset")
                try:
                    available_experiments = set(
                        generic_dataset["experiment_id"].values.astype(str).tolist()
                    )
                    if all(
                        requested_experiment_id in available_experiments
                        for requested_experiment_id in requested_experiment_ids
                    ):
